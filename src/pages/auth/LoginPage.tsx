...
export function LoginPage() {
  const { login, currentUser, error, loading, clearError } = useAuth()
  ...
  const handleDemoLogin = async (
    role: 'elderly' | 'caregiver' | 'healthWorker'
  ) => {
    const creds = {
      elderly: {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, #70B77E, transparent)',
        }}
      />
          ...
          {isDemoMode && (
            <div className="bg-accent/15 border border-accent/30 rounded-2xl p-4 mb-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(
                  ['elderly', 'caregiver', 'healthWorker'] as const
                ).map((role) => (
                  <button
                    key={role}
                    type="button"
