...
import { Mic, MicOff, Volume2, RotateCcw, Clock, Sparkles, Send, ArrowLeft, Bot, User } from 'lucide-react'
...
export function VoicePage() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      sender: 'assistant',
      text: 'Namaste! I am Cogniva, your voice companion. Tap the big microphone button to speak with me.',
  ...
  const handleSpeechResult = async (userText: string) => {
    ...
    try {
      ...
      const aiResponse = await aiService.chat(userText, history)
      ...
      const fallbackMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        sender: 'assistant',
        text: 'I heard you! Today is a bright day. Remember to take your scheduled medicines and drink a glass of warm water.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      }
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-24 px-4 pt-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-center flex-1">
          ...
          <p className="text-slate-600 text-sm">Your AI voice companion for help and conversation</p>
        </div>
        <div className="w-20" />
      </div>
      ...
      <Card className="p-8 text-center bg-gradient-to-b from-teal-50 to-white border-2 border-teal-200 shadow-md">
        <p className="text-lg font-medium text-slate-700 mb-6">
          {isListening
            ...
            : isProcessing
            ? 'Cogniva is thinking...'
            : 'Tap the big button and talk to me'}
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Button
          variant="outline"
          className="h-14 text-base font-semibold border-teal-300 text-teal-800 bg-teal-50 hover:bg-teal-100 flex items-center justify-center gap-2"
          onClick={() => handleQuickAction('Tell me a short story from North East India')}
        >
        ...
        <Button
          variant="outline"
          className="h-14 text-base font-semibold border-teal-300 text-teal-800 bg-teal-50 hover:bg-teal-100 flex items-center justify-center gap-2"
          onClick={() => handleQuickAction('Repeat my medicine schedule for today')}
        >
